...
static char shortopts[] = "?atlgopmrc";
...
int ReadOpts(int argc, char **argv, struct pgm_options *options) {
  ...
  while ((ch = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1) {
    ...
    case 'o':
      ...
      break;
    case 'g':
      options->sensor_type  = SENSOR_TYPE_GYROSCOPE;
      break;
    case 'p':
    ...
    case 'm':
      ...
      break;
    case 'r':
      options->sensor_type  = SENSOR_TYPE_ROTATION_VECTOR;
      break;
    case 'c':
...
void DisplaySensorData(int sensor_type, const sensors_event_t *data) {
  switch (sensor_type) {
    ...
    case SENSOR_TYPE_ROTATION_VECTOR: {
      printf("Rotation Vector: x = %f, y = %f, z = %f, w = %f\n",
             data->data[0], data->data[1], data->data[2], data->data[3]);
      }
      break;
    case SENSOR_TYPE_GYROSCOPE: {
      printf("Gyroscope Vector: x = %f, y = %f, z = %f\n",
...
int main(int argc, char* argv[]) {
  ...
  if (ret || !sensor_module) {
    fprintf(stderr, "Failed to load 1 %s module: %s\n",
  ...
  for (int i = 0; i < sensor_count; i++) {
    ...
    printf("\tsupported sensors type: %d\n", sensor->type);
    ...
    for (int i = 0; i < kNumSamples; ) {
      ...
      if (!event_count) {
        ...
        for(int j = 0; j < kNumEvents; j++) {
          DisplaySensorData(data[j].type, &data[j]);
        }
        i++;
      }
      // sleep(kWaitTimeSecs);
